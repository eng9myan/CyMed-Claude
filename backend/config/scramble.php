...
return [
    ...
    'info' => [
        ...
        'description' => <<<'MD'
# CyMed Healthcare ERP API

Enterprise healthcare ERP & EMR platform for multi-facility deployments in Saudi Arabia and MENA.

## Authentication

All endpoints (except `/auth/login` and `/auth/register`) require a **Bearer token** obtained from `POST /api/v1/auth/login`.

```
Authorization: Bearer <access_token>
```

Tokens expire after **8 hours**. Refresh via `POST /api/v1/auth/refresh`.

## FHIR R4

FHIR R4-compliant resources are available at `/api/v1/fhir/r4/{resourceType}`.
Supported resource types: `Patient`, `Encounter`, `Observation`, `MedicationRequest`, `Condition`, `Procedure`, `AllergyIntolerance`, `DiagnosticReport`.
...
- **NPHIES**: Insurance claim submission via `/api/v1/insurance/claims`
- **ZATCA**: e-Invoicing fields included on all invoice responses (`zatca_uuid`, `zatca_qr_code`)
MD,
    ],

    'ui' => [
        'title' => 'CyMed API Reference',
    ],
    ...
    'middleware' => [
        'web',
        // Restrict to local/staging; remove for public docs
        app()->isProduction() ? RestrictedDocsAccess::class : null,
    ],
